Public Class OrdemEntrega
    ...
    Private Sub CódigoCliente_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles CódigoCliente.Enter
        If Me.CódigoCliente.Text = "" Then
            ...
            ElseIf PedidoVenda.Visible = True Then
    ...
    Private Sub AchaClientePedido()
        ...
        Dim Sql As String = "SELECT Pedido.PedidoSequencia, Pedido.CódigoCliente, Clientes.Nome, Pedido.DataPedido, Pedido.TotalPedido, Pedido.PedidoTipo, Funcionários.Nome as Vendedor, Pedido.StatusAndamentos FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE (((Pedido.PedidoSequencia)=" & Me.Pedido.Text & ") AND ((Pedido.Empresa)=" & CodEmpresa & ") AND ((Pedido.Inativo)=False) AND ((Pedido.Confirmado)=True) AND ((Pedido.PedidoTipo)='VENDA'));"
        ...
        If ds.Tables("Pedido").Rows.Count = 1 Then

            Me.CódigoCliente.Text = ds.Tables("Pedido").Rows(0)("CódigoCliente")
            Me.NomeCliente.Text = ds.Tables("Pedido").Rows(0)("Nome")
