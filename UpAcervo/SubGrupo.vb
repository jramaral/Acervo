Public Class SubGrupo
    ...
    Private Sub SalvarBT_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SalvarBT.Click
        ...
        If Me.Codigo.Text = "" Then
            MsgBox("O Código do Sub-Grupo não foi informado, favor verificar.", 64, "Validação de Dados")
            Me.Codigo.Focus()
            Exit Sub
        ElseIf Me.SubGrupoDesc.Text = "" Then
            MsgBox("A  Descrição do Sub-Grupo não foi informado, favor verificar.", 64, "Validação de Dados")
            Me.SubGrupoDesc.Focus()
            Exit Sub
        End If
        ...
        If Operation = INS Then
            ...
            UltimoReg()
            Dim Sql As String = "INSERT INTO SubGrupo (Codigo, SubGrupoDesc, Empresa) VALUES (@1, @2, @3)"
            Dim cmd As New OleDb.OleDbCommand(Sql, CNN)

            cmd.Parameters.Add(New OleDb.OleDbParameter("@1", Nz(Me.Codigo.Text, 1)))
            cmd.Parameters.Add(New OleDb.OleDbParameter("@2", Nz(Me.SubGrupoDesc.Text, 1)))
        ...
        ElseIf Operation = UPD Then
            ...
            Dim Sql As String = "Update SubGrupo set SubGrupoDesc = @2, Empresa = @3 Where Codigo = " & Me.Codigo.Text
            Dim cmd As New OleDb.OleDbCommand(Sql, CNN)
    ...
    Public Sub UltimoReg()
        ...
        With Cmd
            .Connection = Cnn
            .CommandTimeout = 0
            .CommandText = "Select max(Codigo) from SubGrupo"
    ...
    Private Sub SubGrupo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Ação.Desbloquear_Controle(Me, False)
        Me.TxtProcura.Enabled = True
        Me.SubGrupoDesc.Focus()
    End Sub
    ...
    Private Sub ExcluirBT_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ExcluirBT.Click
        ...
        Dim SQL As String = "Select * From Produtos Where SubGrupo = " & Me.Codigo.Text
        Dim Cmd As New OleDb.OleDbCommand(SQL, Cnn)
        ...
        Else
            If MsgBox("Deseja realmente excluir este Item ?", 36, "Validação de Dados") = 6 Then
                SQL = "Delete * From SubGrupo Where Codigo = " & Me.Codigo.Text
...
#Region "Rotina de Procura"
    ...
    Private Sub Buscar(ByVal Opt As Integer)
        ...
        Select Case Opt
            Case 1
                Sql = "Select SubGrupo.Codigo, SubGrupo.SubGrupoDesc From SubGrupo Where SubGrupo.Empresa = " & CodEmpresa & " And Codigo = " & Me.TxtProcura.Text & " Order by SubGrupo.Marca"
            Case 2
                Sql = "Select SubGrupo.Codigo, SubGrupo.SubGrupoDesc From SubGrupo Where SubGrupo.Empresa = " & CodEmpresa & "  And SubGrupo.SubGrupoDesc like '%" & Me.TxtProcura.Text & "%' Order by SubGrupo.SubGrupoDesc"
            Case 3
                Sql = "Select SubGrupo.Codigo, SubGrupo.SubGrupoDesc From SubGrupo Where SubGrupo.Empresa = " & CodEmpresa & " Order by SubGrupo.SubGrupoDesc"
        End Select
    ...
    Private Sub Lista_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Lista.CellDoubleClick
        ...
        Me.SubGrupoDesc.Text = Me.Lista.CurrentRow.Cells(1).Value
        Me.Codigo.Enabled = False
        Me.SubGrupoDesc.Enabled = False
        Operation = UPD
