...
Public Class Receber
    ...
    Sub EncheGrid()
        ...
        Select Case cboFiltros.SelectedIndex
            ...
            Case 0 'por todos
                sb.Append("SELECT id, documento, NumeroBoleto, notafiscal, Locacao.DataLoc, receber.cliente, datadocumento, vencimento, valordocumento, prorrogada, virtual, ContaPerdida, Origem, IdAgrupamento, contacorrente, receber.LocalPgto, receber.codcliente")
                sb.Append(" FROM Receber")
                sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
                sb.Append(" WHERE Baixado = False")
                sb.Append(" and receber.Empresa = " & CodEmpresa & "")
                sb.Append(" and receber.Inativo = False")
                sb.Append(" and CartaoCredito = False")
                sb.Append(" Order by DataLoc")

            Case 1 'por data
                If Me.txtProcura.Text = "" Then
                    MsgBox("Data não pode ser nulo", 48, "ATENÇÃO")
                    Exit Sub
                End If
                sb.Append("SELECT id, documento, NumeroBoleto, notafiscal, Locacao.DataLoc, receber.cliente, datadocumento, vencimento, valordocumento, prorrogada, virtual, ContaPerdida, Origem, IdAgrupamento, contacorrente, receber.LocalPgto, receber.codcliente")
                sb.Append(" FROM Receber")
                sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
                sb.Append(" WHERE Vencimento =#" & Format(CDate(Me.txtProcura.Text), "MM/dd/yyyy") & "#")
                sb.Append(" and Baixado = False")
                sb.Append(" and receber.Empresa = " & CodEmpresa & "")
                sb.Append(" and receber.Inativo = False")
                sb.Append(" and CartaoCredito = False")
            ...
            Case 2 'por nome
                If Me.txtProcura.Text = "" Then
                    MsgBox("Cliente não pode ser nulo", 48, "ATENÇÃO")
                    Exit Sub
                End If
                sb.Append("SELECT id, documento, NumeroBoleto, notafiscal, Locacao.DataLoc, receber.cliente, datadocumento, vencimento, valordocumento, prorrogada, virtual, ContaPerdida, Origem, IdAgrupamento, contacorrente, receber.LocalPgto, receber.codcliente")
                sb.Append(" FROM Receber")
                sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
                sb.Append(" WHERE Receber.Cliente Like '%" & Me.txtProcura.Text & "%'")
                sb.Append(" and Baixado = False")
                sb.Append(" and receber.Empresa = " & CodEmpresa & "")
                sb.Append(" and receber.Inativo = False")
                sb.Append(" and CartaoCredito = False")
            ...
            Case 3 'por numero de boletos
                sb.Append("SELECT id, documento, NumeroBoleto, notafiscal, Locacao.DataLoc, receber.cliente, datadocumento, vencimento, valordocumento, prorrogada, virtual, ContaPerdida, Origem, IdAgrupamento, contacorrente, receber.LocalPgto, receber.codcliente")
                sb.Append(" FROM Receber")
                sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
                sb.Append(" WHERE NumeroBoleto like " & Me.txtProcura.Text & "")
                sb.Append(" and Baixado = False")
                sb.Append(" and receber.Empresa = " & CodEmpresa & "")
                sb.Append(" and receber.Inativo = False")
                sb.Append(" and CartaoCredito = False")
                sb.Append(" Order by Vencimento")
            Case 4 'por numero de boletos
                sb.Append("SELECT id, documento, NumeroBoleto, notafiscal, Locacao.DataLoc, receber.cliente, datadocumento, vencimento, valordocumento, prorrogada, virtual, ContaPerdida, Origem, IdAgrupamento, contacorrente, receber.LocalPgto, receber.codcliente")
                sb.Append(" FROM Receber")
                sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
                sb.Append(" WHERE NumeroBoleto like " & Me.txtProcura.Text & "")
    ...
    Private Sub MostrarContasProToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MostrarContasProToolStripMenuItem.Click
        ...
        sb.Append(" FROM Receber")
        sb.Append(" LEFT JOIN Locacao ON Receber.PedidoProdutos = Locacao.IdLoc")
        sb.Append(" WHERE Vencimento <= #" & DataDia & "#")
        sb.Append(" And Baixado = False")
        sb.Append(" And receber.Empresa = " & CodEmpresa & "")
        sb.Append(" And receber.Inativo = False")
        sb.Append(" And prorrogada=true")
        sb.Append(" And CartaoCredito = false")
        sb.Append(" order by Vencimento")
    ...
    Private Sub AlterarContaCorrenteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AlterarContaCorrenteToolStripMenuItem.Click
        TRVDados(UserAtivo, "BoletoIndicarConta")
        ...
        If Ina = True Then
            ...
            Exit Sub
        Else
            My.Forms.BoletoIndicarConta.ShowDialog()
            EncheGrid()
        End If
    ...
    Private Sub DividirToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DividirToolStripMenuItem.Click

        Dim i As Integer = dgvItem.CurrentRow.Index

        My.Forms.ReceberDecompor.txt_id_linha.Text = dgvItem.CurrentRow.Cells("g_id").Value
        My.Forms.ReceberDecompor.txt_valor_documento.Text = dgvItem.CurrentRow.Cells("g_valor").Value
        My.Forms.ReceberDecompor.txt_doc.Text = dgvItem.CurrentRow.Cells("g_documento").Value
        My.Forms.ReceberDecompor.ShowDialog()

        BtnPesquisar_Click(sender, e)


        dgvItem.Rows(i).Selected = True
        dgvItem.FirstDisplayedScrollingRowIndex = i
