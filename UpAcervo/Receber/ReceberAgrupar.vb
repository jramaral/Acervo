...
Public Class ReceberAgrupar
    ...
    Private Sub btDesfazer_Click(sender As Object, e As EventArgs) Handles btDesfazer.Click
        ...
        Try
            Dim ds As New DataSet
            Sql = "Select * FROM Receber WHERE Receber.IdAgrupamento = " & Me.IdAgrupar.Text & " AND Receber.OriginalParcial = 'O' And Agrupada = False and Baixado = True"
            Dim cmdVer As New OleDbCommand(Sql, Cnn)
            cmdVer.Transaction = Transacao
            Dim daVer As New OleDbDataAdapter(cmdVer)
            daVer.Fill(ds, "Verificar")

            If ds.Tables("Verificar").Rows.Count > 0 Then
                MessageBox.Show("Ja existe baixa de documento para este agrupamento, agrupamento não pode ser desfeito.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Cnn.Close()
                Exit Sub
            End If
    ...
    Private Sub btnImprimir_Click(sender As Object, e As EventArgs) Handles btnImprimir.Click
        ...
        formula = "{ReceberAgrupar.IdAgrupar} = " & Me.IdAgrupar.Text & " and {Receber.Agrupada}"
        Dim t As String
        If NzZero(Me.IdAgrupar.Text) > 0 Then
            RelatorioCarregar = "RelAgrupamentoReceber.rpt"
            t = "Relatório de agrupamento a receber"
