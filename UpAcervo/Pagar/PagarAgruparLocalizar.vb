...
Public Class PagarAgruparLocalizar
    ...
    Private Sub PagarAgruparLocalizar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BuscarAgrupamentos(ByVal Opt As String)
        ...
        Select Case Opt
            Case 1
                Sql = "SELECT PagarAgrupar.IdAgrupar, PagarAgrupar.DataLancamento, Fornecedor.RazãoSocial, PagarAgrupar.ValorAgrupado, PagarAgrupar.Confirmado FROM PagarAgrupar INNER JOIN Fornecedor ON PagarAgrupar.Fornecedor = Fornecedor.CódigoFornecedor WHERE (((PagarAgrupar.IdAgrupar)=" & Me.TxtProcura.Text & ") AND ((PagarAgrupar.Confirmado)=True) AND ((PagarAgrupar.Inativo)=False));"
            Case 2
                Sql = "SELECT PagarAgrupar.IdAgrupar, PagarAgrupar.DataLancamento, Fornecedor.RazãoSocial, PagarAgrupar.ValorAgrupado, PagarAgrupar.Confirmado FROM PagarAgrupar INNER JOIN Fornecedor ON PagarAgrupar.Fornecedor = Fornecedor.CódigoFornecedor WHERE (((PagarAgrupar.DataLancamento)=#" & Format(CDate(Me.TxtProcura.Text), "MM/dd/yyy") & "#) AND ((PagarAgrupar.Confirmado)=True) AND ((PagarAgrupar.Inativo)=False));"
            Case 3
                Sql = "SELECT PagarAgrupar.IdAgrupar, PagarAgrupar.DataLancamento, Fornecedor.RazãoSocial, PagarAgrupar.ValorAgrupado, PagarAgrupar.Confirmado FROM PagarAgrupar INNER JOIN Fornecedor ON PagarAgrupar.Fornecedor = Fornecedor.CódigoFornecedor WHERE (((Fornecedor.RazãoSocial) Like '%" & Me.TxtProcura.Text & "%') AND ((PagarAgrupar.Confirmado)=True) AND ((PagarAgrupar.Inativo)=False));"
        End Select
    ...
    Private Sub Lista_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Lista.CellDoubleClick
        ...
        If My.Forms.PagarAgrupar.Visible = True Then My.Forms.PagarAgrupar.IdAgrupar.Text = vID
