...
Public Class Caixa
    ...
    Private Sub CarregaItens()
        ...
        Sql = "SELECT Id, ContaCorrente, DataLancamento, Documento, DataDocumento, Favorecido, Historico, ValorDocumento, IdLancamento, IdProcura, Tipo, ContaBalancete,ContasG3.DescContaGrupo3, TipoLancamento FROM LancamentoBanco Left JOIN ContasG3 ON LancamentoBanco.ContaBalancete = ContasG3.ContaGrupo3 WHERE (((LancamentoBanco.[DataLancamento])=#" & Format(CDate(Me.DataCaixa.Text), "MM/dd/yyy") & "#) AND ((LancamentoBanco.ContaCorrente)='" & Me.CaixaCod.Text & "') AND ((LancamentoBanco.Tipo)='CAIXA') AND ((LancamentoBanco.Empresa)=" & CodEmpresa & "))"

        Dim da = New OleDb.OleDbDataAdapter(Sql, Cnn)
        Dim ds As New DataSet
        da.Fill(ds, "Table")

        VlrLancamentos = NzZero(ds.Tables("Table").Compute("sum(ValorDocumento)", "TipoLancamento <> 'R'"))
    ...
    Private Sub btLancamentos_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btLancamentos.Click
        ...
        If Me.DisplayFechado.Visible = True Then
            ...
            Exit Sub
        Else
            If String.IsNullOrEmpty(CaixaCod.Text) Then
                MessageBox.Show("Código do caixa  não pode ser nulo.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
                CaixaCod.Focus()
                Exit Sub
            End If
