...
Public Class FuncionarioProcura
    ...
    Private Sub BuscarFuncionario(ByVal Opt As String)
        ...
        Select Case Opt
            Case 1
                Sql = "SELECT Funcionários.CódigoFuncionário, Funcionários.Nome, Profissao.Descrição, Funcionários.inativo FROM Funcionários LEFT JOIN Profissao ON Funcionários.Profissão = Profissao.Codigo WHERE (((Funcionários.CódigoFuncionário)=" & Me.TxtProcura.Text & ")) and Funcionários.Empresa =" & CodEmpresa & " Order by Funcionários.Nome"
            Case 2
                Sql = "SELECT Funcionários.CódigoFuncionário, Funcionários.Nome, Profissao.Descrição, Funcionários.inativo FROM Funcionários LEFT JOIN Profissao ON Funcionários.Profissão = Profissao.Codigo WHERE (((Funcionários.Nome) like '%" & Me.TxtProcura.Text & "%')) and Funcionários.Empresa =" & CodEmpresa & " Order by Funcionários.Nome"
            Case 3
                Sql = "SELECT Funcionários.CódigoFuncionário, Funcionários.Nome, Profissao.Descrição, Funcionários.Empresa,  Funcionários.inativo FROM Funcionários LEFT JOIN Profissao ON Funcionários.Profissão = Profissao.Codigo WHERE (((Funcionários.Empresa)=" & CodEmpresa & "));"
        End Select
