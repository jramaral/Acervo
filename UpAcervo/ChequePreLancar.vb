Public Class ChequePreLancar
    ...
    Private Sub btFechar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btFechar.Click
        ...
        If My.Forms.ReceberBaixa.Visible = True Then
            If FormatCurrency(NzZero(Me.VlrLançado.Text), 3) = FormatCurrency(0, 3) Then
                MsgBox("Não foi lançado nenhum valor de cheque-pré o sistema ira anular a Operação.", 64, "Validação de Dados")
        ...
        If NzZero(Me.VlrLançar.Text) <> NzZero(Me.VlrLançado.Text) Then
    ...
    Private Sub ChequePreLancar_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If My.Forms.ReceberBaixa.Visible = True Then
            ...
            Me.VlrLançar.Text = FormatCurrency(CDbl(My.Forms.ReceberBaixa.ValorLiquido.Text), 2)
        ...
        If My.Forms.PedidoVendaConfirmar.Visible = True Then
            ...
            'Me.Documento.Text = My.Forms.ReceberBaixa.Documento.Text
            Me.VlrLançar.Text = FormatCurrency(CDbl(My.Forms.PedidoVendaConfirmar.ValorOutros.Text), 2)
            Vendedor = My.Forms.PedidoVenda.CódigoFuncionário.Text
    ...
    Private Sub SalvarReceber()
        ...
        If CDate(Me.Vencimento.Text) <= CDate(My.Forms.ReceberBaixa.Recebimento.Text) Then
            MessageBox.Show("A Data de vencimento do Cheque não pode ser menor que a data da baixa.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Me.Vencimento.Focus()
            Exit Sub
        End If
        ...
        If Operation = INS Then
            ...
            Catch ex As Exception
                If Err.Number = 5 Then
                    MessageBox.Show("Este cheque já esta cadastrado no sistema." & Chr(13) & "Caso você esteja fazendo baixa de mais de uma conta com um único cheque você deve sempre colocar um numero de sequencia após o nr. do cheque" & Chr(13) & "Ex: -1, Caso seja três lançamento -2 e consequentemente.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
    ...
    Private Sub SalvarPedido()
        ...
        ElseIf Me.EntreguePor.Text = "" Then
            ...
            Exit Sub
        ElseIf Me.ValorCh.Text = "" Then
            MsgBox("O Valor do cheque não pode ser nulo", 64, "Validação de Dados")
            Me.ValorCh.Focus()
            Exit Sub
        ElseIf Me.Vencimento.Text = "" Then
            MsgBox("O Vencimento do cheque não pode ser nulo", 64, "Validação de Dados")
        ...
        If CDate(Me.Vencimento.Text) <= CDate(My.Forms.PedidoVenda.DataPedido.Text) Then
            MessageBox.Show("A Data de vencimento do Cheque não pode ser menor que a data do Pedido.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Me.Vencimento.Focus()
            Exit Sub
        End If
        ...
        If Operation = INS Then
            ...
            cmd.Parameters.Add(New OleDb.OleDbParameter("@10", "CHEQUE-PRE"))
            cmd.Parameters.Add(New OleDb.OleDbParameter("@11", Nz(Me.Documento.Text, 1)))
            cmd.Parameters.Add(New OleDb.OleDbParameter("@12", Nz(My.Forms.PedidoVenda.CódigoCliente.Text, 3)))
