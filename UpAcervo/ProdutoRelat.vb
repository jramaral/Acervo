...
Public Class ProdutoRelat
    ...
    Public varPedidos() As String
    Public isNum As Boolean = False
    ...
    Private Sub Visualizar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Visualizar.Click
        ...
        If Me.A33.Checked = True Then
            ...
            'passa o nome do relatório
            RelatorioCarregar = "RelProdutoGrupoImpostosConferencia"
            My.Forms.VisualizadorRelatorio.ShowDialog()

        End If

        'Lista de Preços  por Marca
        If Me.A34.Checked = True Then
            RelatorioCarregar = "RelListaPrecoMarca"
            My.Forms.VisualizadorRelatorio.ShowDialog()
        End If

        'Relatóro Saida de Produtos [Marcas], Saida de Produtos [Grupos]
        If Me.A40.Checked Or A41.Checked Then

            'verifica se pelos menos uma das opçoes tem dados para gerar o relatorios, 
            'Condição: pelo menos uma das opções são necessários.
            If String.IsNullOrEmpty(Me.txtNumeroPedidos.Text) And String.IsNullOrEmpty(Me.DataInicial.Text & Me.DataFinal.Text) Then
                MessageBox.Show("Pelo menos uma das opções precisa ser inserida, [Pedidos] Ou [Período]", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            'Aqui verifica se as duas opções estão inseridas dados, 
            'Condição: Somente uma deverá ser inserida e alertará o usuário.
            If Not String.IsNullOrEmpty(Me.txtNumeroPedidos.Text) And Not String.IsNullOrEmpty(Me.DataInicial.Text) Then
                MessageBox.Show("Somente uma das opções são válidas!, [Pedidos] Ou [Período]", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            'Verifica se tem pelo menos um numero
            'If Not isNum Then
            '    MessageBox.Show("Você precisa digitar pelos menos um numero", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            '    Exit Sub
            'End If



            'retira o ultimo elemento caso exista
            Dim v As String
            v = txtNumeroPedidos.Text.TrimEnd(";")

            'faz a separação dos pedidos 
            varPedidos = v.Split(";")
        ...
        'relatorio com impostos para conferencia cst/ pis e cofins
        If Me.A43.Checked = True Then
            RelatorioCarregar = "RelProdutoGrupoImpostos.rpt"
        ...
        'Relaotiros de sida vendedor/marca  e cidade marca
        'Relatóro Saida de Produtos [Marcas], Saida de Produtos [Grupos]
        If Me.rbtVendedorMarca.Checked Or rbtCidadeMarca.Checked Then
            Dim f As New ClassView.cView
            Dim formula As String = String.Empty

            formula = "{Pedido.DataFechamento}>=Date (" & Format(CDate(DataInicial.Text), "yyyy,MM,dd") & ") and {Pedido.DataFechamento}<=date(" & Format(CDate(DataFinal.Text), "yyyy,MM,dd") & ") and not {Pedido.Inativo} and {Pedido.Empresa}=" & CodEmpresa
            Dim t As String
            If Me.rbtVendedorMarca.Checked Then
                RelatorioCarregar = "Prod_VendFuncionarioMarca.rpt"
                t = "Produtos por Vendedor e Marca"
            Else
                RelatorioCarregar = "Prod_VndMarcaCidade.rpt"
                t = "Produtos por Cidade e Marca"
            End If

            f.frm(DirRelat & RelatorioCarregar, LocalBD & Nome_BD, SenhaBancoDados, t, formula)


        End If
        If Me.A44.Checked = True Then
            RelatorioCarregar = "RelprodutosInativosPorMarca"
            My.Forms.VisualizadorRelatorio.ShowDialog()
    ...
    Private Sub A1_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles A1.CheckedChanged, A2.CheckedChanged, A3.CheckedChanged, A5.CheckedChanged, A7.CheckedChanged, A8.CheckedChanged, A9.CheckedChanged, A20.CheckedChanged, A21.CheckedChanged, A22.CheckedChanged, A30.CheckedChanged, A31.CheckedChanged, A32.CheckedChanged, A33.CheckedChanged, A34.CheckedChanged, A40.CheckedChanged, A41.CheckedChanged, A42.CheckedChanged, rdb_qtd_vendida.CheckedChanged, rbtVendedorMarca.CheckedChanged, rbtCidadeMarca.CheckedChanged, A44.CheckedChanged
        ...
        If Me.A40.Checked Or A41.Checked Then
            ...
            'Me.PainelPeriodo.Location = New System.Drawing.Point(7, 282)
            'Me.painelPedido.Location = New System.Drawing.Point(7, 341)
        ...
        If Me.A43.Checked = True Then
            ...
            Me.GrupoDesc.Visible = True
        End If
        If rbtCidadeMarca.Checked Or rbtVendedorMarca.Checked Then
            Me.PainelListaPreco.Visible = False
            Me.PainelGrupo.Visible = False
            Me.Grupo.Clear()
            Me.GrupoDesc.Clear()
    ...
    Private Sub txtNumeroPedidos_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtNumeroPedidos.KeyPress
