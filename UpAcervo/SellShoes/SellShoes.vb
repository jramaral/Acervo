...
Public Class SellShoes
    ...
    Dim vNumeroOrcamento As Integer = 0
    ...
    Private Sub SellShoes_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If AcharVenda = False Then
            ...
            If Me.PedidoTipo.Text = "VENDA BRINDE" Then
                Me.TipoPgto.Enabled = False
                Me.CodPgto.Enabled = False
                Me.Contrato.Enabled = False
                AcharFormaAnexa()
            ...
            If Me.PedidoTipo.Text = "DEBITO INTERNO" Then
                Me.TipoPgto.Enabled = False
                Me.CodPgto.Enabled = False
                Me.Contrato.Enabled = False
                AcharFormaAnexa()
    ...
    Private Sub AcharFormaAnexa()

        Dim ds As New DataSet
        Dim Cnn As New OleDb.OleDbConnection(New Conectar().ConectarBD(LocalBD & Nome_BD))

        Dim Sql As String = "SELECT Empresa.CódigoEmpresa, Empresa.AnexoPgto, AnexarForma.Tp_Codigo, AnexarForma.Fp_Codigo FROM Empresa INNER JOIN AnexarForma ON Empresa.AnexoPgto = AnexarForma.CodigoAnexo WHERE (((Empresa.CódigoEmpresa)=" & CodEmpresa & "));"

        Dim Da As New OleDb.OleDbDataAdapter(Sql, Cnn)
        Da.Fill(ds, "Anexo")

        If ds.Tables("Anexo").Rows.Count = 1 Then

            Me.TipoPgto.SelectedValue = ds.Tables("Anexo").Rows(0)("Tp_Codigo")
            EncheCodPgto()
            Me.CodPgto.SelectedValue = ds.Tables("Anexo").Rows(0)("Fp_Codigo")
        Else
            MessageBox.Show("Não foi informado o anexo de pagamento nas configurações da empresa, favor verificar", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Cnn.Close()
            Me.Close()
            Me.Dispose()
        End If
...
#Region "Rotinas de Find"
    ...
    Private Sub LocalizaProdutoCodBarraSemNumeros(ByVal IdProduto As Integer)
        ...
        If DR.HasRows Then
            ...
            If Me.PedidoTipo.Text = "VENDA BRINDE" Or Me.PedidoTipo.Text = "DEBITO INTERNO" Then
                Me.Unitario.Text = FormatNumber(NzZero(DR.Item("custo2")), 2)
            Else
                If Me.TpComercio.Text = "VAREJO" Then
                    Me.Unitario.Text = FormatNumber(NzZero(DR.Item("ValorVenda")), 2)
                Else
    ...
    Private Sub LocalizaProdutoCodBarraSemNumerosLeitor(ByVal Var_CodBarra As String, QtdVar As Double)

        If Var_CodBarra = "" Then Exit Sub

        Dim CNN As New OleDb.OleDbConnection(New Conectar().ConectarBD(LocalBD & Nome_BD))
        CNN.Open()

        Dim Sql As String = "SELECT Produtos.* FROM Produtos WHERE Produtos.Inativo=False And Produtos.CodigoBarra = '" & Var_CodBarra & "'"
        ...
        If DR.HasRows Then
            ...
            Me.Descricao.Text = DR.Item("Descrição") & ""
            Me.Numero.Text = 0
            Me.Qtd.Text = FormatNumber(QtdVar, 4)
...
#End Region
            ...
            Else
                If Var_EnterCodBarra = True Then
                    ...
                    If Asterisco.Length = 1 Then
                        LocalizaProdutoCodBarraSemNumerosLeitor(Asterisco(0), 1)
                    Else
                        LocalizaProdutoCodBarraSemNumerosLeitor(Asterisco(1), Asterisco(0))
                    End If
                Else
    ...
    Private Sub btEnviarCaixa_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btEnviarCaixa.Click
        ...
        If Me.PedidoTipo.Text = "VENDA BRINDE" Or Me.PedidoTipo.Text = "DEBITO INTERNO" Then
            If NzZero(Me.ValorCheque.Text) <> 0 Then
                MessageBox.Show("Para este tipo de venda não pode usar opção de faturamento." & Chr(13) & "Favor verificar a forma de pagamento na configuração da empresa." & Chr(13) & "Ou verifique com o Administrador.", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.Close()
                Me.Dispose()
                Exit Sub
            End If
        ...
        If CDbl(NzZero(vTotal)) <> CDbl(NzZero(Me.TotalPedido.Text)) Then
            ...
            Else
                MessageBox.Show("O total dos pagamentos estão diferente do Total do Pedido, Verifique...", "Validação de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.ValorFaturar.Text = Me.TotalPedido.Text
                ValorFaturar_Leave(sender, e)
            End If
        End If
        ...
        Try
            ...
            Cmd.Parameters.Add(New OleDb.OleDbParameter("@11", TpEntrega))
            ...
            Cmd.Parameters.Add(New OleDb.OleDbParameter("@31", vNumeroOrcamento))
        ...
        If UsaSellShoes = False Then
            Try
                ...
                cl.strPedOriginal = DevNumero.Text
                cl.Impressao(bImprimePedido) 'Faz a impresão
    ...
    Private Sub VerDebitoCliente()
        ...
        ' (ATUAL apresenta erro de valores )Dim sql As String = "SELECT Receber.CodCliente AS Cliente, Sum(Receber.ValorDocumento) AS Total FROM(Receber) WHERE (((InStr(1,[Documento],'-C'))=0) AND ((Receber.Recebimento) Is Null) AND ((Receber.Baixado)=False) AND ((Receber.Empresa)=" & CodEmpresa & ")) GROUP BY Receber.CodCliente HAVING (((Receber.CodCliente)=" & Me.xCodCliente.Text & ")); UNION SELECT ArquivoCheque.Cliente, Sum(ArquivoCheque.ValorCh) AS Total FROM ArquivoCheque WHERE (((ArquivoCheque.DataBaixa) Is Null) AND ((ArquivoCheque.Empresa)=" & CodEmpresa & ")) GROUP BY ArquivoCheque.Cliente HAVING (((ArquivoCheque.Cliente)=" & Me.xCodCliente.Text & "));"
        ''Dim Sql As String = "SELECT Receber.CodCliente AS Cliente, Sum(Receber.ValorDocumento) AS Total FROM Receber WHERE (((Receber.Recebimento) Is Null) AND ((Receber.Baixado)=False) And ((Receber.Inativo)=False) AND ((Receber.CodigoContrato) Is Null) AND ((Receber.Empresa)=" & CodEmpresa & ")) GROUP BY Receber.CodCliente HAVING (((Receber.CodCliente)=" & Me.xCodCliente.Text & ")); UNION SELECT ArquivoCheque.Cliente, Sum(ArquivoCheque.ValorCh) AS Total FROM ArquivoCheque WHERE (((ArquivoCheque.DataBaixa) Is Null) AND ((ArquivoCheque.Empresa)=" & CodEmpresa & ")) GROUP BY ArquivoCheque.Cliente HAVING (((ArquivoCheque.Cliente)=" & Me.xCodCliente.Text & "));"
        Dim Sql As String = "SELECT Receber.CodCliente AS Cliente, Sum(Receber.ValorDocumento) AS Total FROM Receber WHERE (((Receber.Recebimento) Is Null) AND ((Receber.Baixado)=False) And ((Receber.Inativo)=False) AND ((Receber.CodigoContrato) Is Null) AND ((Receber.Empresa)=" & CodEmpresa & ")) GROUP BY Receber.CodCliente HAVING (((Receber.CodCliente)=" & Me.xCodCliente.Text & ")); UNION SELECT ArquivoCheque.Cliente, Sum(ArquivoCheque.ValorCh) AS Total FROM ArquivoCheque WHERE (((ArquivoCheque.DataBaixa) Is Null) AND ((ArquivoCheque.Empresa)=" & CodEmpresa & ")) GROUP BY ArquivoCheque.Cliente HAVING (((ArquivoCheque.Cliente)=" & Me.xCodCliente.Text & "));"
