Public Class SellShoesDevolucao
    ...
    Private Sub EncheListaPedido(ByVal Opc As Integer)
        ...
        If Opc = 1 Then 'Por Pedido
            Sql = "SELECT Pedido.PedidoSequencia, Pedido.CódigoCliente, Clientes.Nome, Pedido.DataPedido, Pedido.TotalPedido, Pedido.PedidoTipo, Pedido.DevNumero,Funcionários.Nome AS Vendedor FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE (((Pedido.PedidoSequencia)=" & Me.txtProcura.Text & ") AND ((Pedido.Empresa)=" & CodEmpresa & ") AND ((Pedido.Inativo)=False) AND ((Pedido.Confirmado)=True) AND ((Pedido.PedidoTipo)='VENDA'));"
        ...
        If Opc = 2 Then 'Por Cliente
            Sql = "SELECT Pedido.PedidoSequencia, Pedido.CódigoCliente,  Clientes.Nome, Pedido.DataPedido, Pedido.TotalPedido, Pedido.PedidoTipo, Pedido.DevNumero, Funcionários.Nome AS Vendedor FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE (((Clientes.Nome) Like '%" & Me.txtProcura.Text & "%') AND ((Pedido.Empresa)=" & CodEmpresa & ") AND ((Pedido.Inativo)=False) AND ((Pedido.Confirmado)=True) AND ((Pedido.PedidoTipo)='VENDA'));"
    ...
    Private Sub SellShoesDevolucao_Load(sender As Object, e As EventArgs) Handles MyBase.Load
