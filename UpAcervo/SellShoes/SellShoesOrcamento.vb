...
Public Class SellShoesOrcamento
    ...
    'Variaveis que servem para passar informa�oes
    ...
    Private Sub btFechar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btFechar.Click
        ...
        If Me.OrcSequencia.Text = "000000" And Me.xCodCliente.Text <> "" Then
            If MessageBox.Show("Deseja sair do Or�amento sem salvar", "Valida��o de Dados", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then
                Exit Sub
            End If
        End If
        Me.Close()
    ...
    Private Sub SellShoesOrcamento_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If AcharOrc = True Then
            ...
            'Desvia para a fun��o e localiza o or�amento
            LocalizarOrc(StrOrc)

            'Dim CN As New OleDb.OleDbConnection(New Conectar().ConectarBD(LocalBD & Nome_BD))
            'CN.Open()

            'Dim Sql As String = "SELECT 0 AS Linha, OrcITEM.CodigoProduto, Produtos.Descri��o, OrcITEM.Numero, OrcITEM.Qtd, OrcITEM.ValorUnitario, OrcITEM.Desconto, OrcITEM.ValorDesconto, OrcITEM.TotalProduto, OrcITEM.CustoMercadoriaVendida FROM OrcITEM INNER JOIN Produtos ON OrcITEM.CodigoProduto = Produtos.CodigoProduto WHERE (((OrcITEM.OrcSequencia)=" & StrOrc & "));"
...
#Region "Rotinas de Find"
    ...
    Public Sub LocalizaCliente(ByVal CodCliente As Integer)
        ...
        If DR.HasRows Then
            ...
            If DR.Item("Inativo") = True Then
                MessageBox.Show("Este cliente foi Inativado, n�o � permitido nenhum tipo de venda para ele, verifique com o Administrador", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
            ...
            Me.xEndereco.Text = DR.Item("Endere�o") & ""
            ...
            If DR.Item("Bloqueado") = True Then
                flag = True
                MessageBox.Show("Este cliente foi bloqueado, verifique com o Administrador ou venda somente a vista", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.ValorCheque.Text = FormatNumber(0, 2)
                Me.ValorCheque.Enabled = False
                Me.ValorFaturar.Text = FormatNumber(0, 2)
                Me.ValorFaturar.Enabled = False
        ...
        Else
            flag = False
            MessageBox.Show("Este cliente n�o consta no cadastro, verifique...", "Valida��o de Daados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.xCodCliente.Clear()
            Me.xCodCliente.Focus()
            Exit Sub
        End If
    ...
    Private Sub LocalizaProdutoCodBarra(ByVal IdProduto As Integer, ByVal IdNumero As String)
        ...
        If DR.HasRows Then
            ...
            Me.Descricao.Text = DR.Item("Descri��o") & ""
                ...
                If DR.Item("ControlaEstoque") = "SIM" Then
                    If Me.PedidoTipo.Text = "VENDA" Then
                        If CDbl(NzZero(DR.Item("QtdEst"))) < CDbl(NzZero(Me.Qtd.Text)) Then
                            MessageBox.Show(" Este Produto n�o possui quantidade suficiente para venda, favor verificar", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            CNN.Close()
                            Me.CodBarra.Clear()
                            Me.CodBarra.Focus()
                            Exit Sub
            ...
            If Me.TpComercio.Text = "VAREJO" Then
                Me.Unitario.Text = FormatNumber(NzZero(DR.Item("ValorVenda")), 2)
            Else
                Me.Unitario.Text = FormatNumber(NzZero(DR.Item("ValorVendaAtacado")), 2)
            End If
        ...
        Else
            DR.Close()
            CNN.Close()
            MessageBox.Show("Produto n�o encontrado, Verifique ou selecione outro", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.CodProduto.Clear()
            Me.Descricao.Clear()
            Me.Numero.Clear()
            Me.Qtd.Clear()
    ...
    Private Sub LocalizaProdutoCodBarraSemNumeros(ByVal IdProduto As String, VarQtd As Double)
        ...
        If DR.HasRows Then
            ...
            Me.Descricao.Text = DR.Item("Descri��o") & ""
            Me.Numero.Text = 0
            Me.Qtd.Text = FormatNumber(VarQtd, 4)
...
#End Region
    ...
    Private Sub SellShoesSellShoesOrcamento_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.F3 Then
            If Me.xCodCliente.Text = "" Then
                MessageBox.Show("Primeiro o usu�rio deve definir o Cliente, para lan�ar os Produtos", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.xCodCliente.Focus()
                Exit Sub
            End If
            HabilitarItens(False)
    ...
    Private Sub AddItem()
        ...
        If DrFindProduto.Length = 0 Then
            ...
            DRnovo("Descri��o") = Me.Descricao.Text
    ...
    Private Sub CodBarra_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles CodBarra.Leave
        ...
        Try
            If UsarGrade = True Then
                ...
                If VarCod.IndexOf("-") = -1 Then
                    MessageBox.Show("N�o existe indexador de numero para este codigo de Produto, a��o sera cancelada.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Me.CodBarra.Clear()
                    Me.CodBarra.Focus()
                    Exit Sub
                End If
            ...
            Else
                If Var_EnterCodBarra = True Then
                    Dim Asterisco() As String = Split(Me.CodBarra.Text, "*")

                    If Asterisco.Length = 1 Then
                        LocalizaProdutoCodBarraSemNumeros(Asterisco(0), 1)
                    Else
                        LocalizaProdutoCodBarraSemNumeros(Asterisco(1), Asterisco(0))
                    End If
                Else
                    Dim Asterisco() As String = Split(Me.CodBarra.Text, "*")
        ...
        Catch ex As Exception
            Select Case Err.Number
                Case 13
                    MsgBox("Erro de convers�o, era esperado o c�digo do produto", MsgBoxStyle.Critical, "Erro")
    ...
    Private Sub DescPercentual_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles DescPercentual.Enter
        If NzZero(Me.ValorProduto.Text) = 0 Then
            MessageBox.Show("O Total do Or�amento n�o pode ser [00,00], para informar desconto", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.CodBarra.Focus()
            Exit Sub
        End If
    ...
    Private Sub ValorVista_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ValorVista.Enter
        'Verifica��o dos Calculos do total da venda
        ...
        If NzZero(Me.TotalOrc.Text) = 0 Then
            MessageBox.Show("N�o existe valor para Or�amento, verifique...", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        Else
    ...
    Private Sub ValorFaturar_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles ValorFaturar.Enter
        ...
        If Me.VendaCrediario.Checked = False And NzZero(Me.ValorFaturar.Text) > 0 Then
            MessageBox.Show("Este Cliente n�o tem permiss�o para fazer Or�amento no credi�rio, verifique...", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.ValorFaturar.Text = FormatNumber(0, 2)
            Me.ValorVista.Focus()
        End If
        ...
        If Me.xNome.Text <> "CONSUMIDOR" Then
            If Me.VendaCrediario.Checked = True And NzZero(Me.ValorFaturar.Text) > CDbl(NzZero(Me.xLimiteSaldo.Text)) Then
                MessageBox.Show("Este Cliente n�o tem Saldo suficiente para o Or�amento no credi�rio, verifique...", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.ValorFaturar.Text = FormatNumber(0, 2)
                Me.ValorVista.Focus()
            End If
        End If
        ...
        If Me.ValorVista.Enabled = True And Me.ValorVista.Text = 0 Then
            MessageBox.Show("A Forma de Pagamento selecionada necessita de um valor de Entrada, Verifique...", "Valida��o de dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Me.ValorVista.Focus()
            Exit Sub
        End If
    ...
    Private Sub ValorFaturar_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles ValorFaturar.Leave
        ...
        If NzZero(ConferirValores) <> NzZero(Me.TotalOrc.Text) Then
            MessageBox.Show("Os Valores indicado est�o diferente do Valor do Or�amento, Verifique...", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.ValorVista.Focus()
            Exit Sub
        End If
        ...
        If NzZero(Me.ValorFaturar.Text) <> 0 Then
            ...
            If Me.TipoPgto.Text = "" Then
                MessageBox.Show("Para o Or�amento o usu�rio deve selecionar um tipo de pagamento.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.TipoPgto.Focus()
                Exit Sub
            End If

            If Me.CodPgto.Text = "" Then
                MessageBox.Show("Para o Or�amento o usu�rio deve selecionar a condi��o de pagamento.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.CodPgto.Focus()
                Exit Sub
            End If
    ...
    Private Sub btEnviarCaixa_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btEnviarCaixa.Click
        ...
        If CalculosOK = False Then
            MessageBox.Show("O usuario deve passar por todos os campos de calculo", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.TipoPgto.Focus()
            Exit Sub
        End If
        ...
        If Me.xCodCliente.Text = "" Then
            MessageBox.Show("N�o foi informado o cliente para o Or�amento.", "Valida��o de Or�amento", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.btEnviarCaixa.Enabled = True
            Me.Cursor = Cursors.Default
            Exit Sub
        End If
        If Me.PedidoTipo.Text = "" Then
            MessageBox.Show("N�o foi informado o Tipo do Or�amento.", "Valida��o de Or�amento", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.btEnviarCaixa.Enabled = True
            Me.Cursor = Cursors.Default
            Exit Sub
        End If
        If Me.Vendedor.Text = "" Then
            MessageBox.Show("N�o foi informado o vendedor para o Or�amento.", "Valida��o de Or�amento", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.btEnviarCaixa.Enabled = True
            Me.Cursor = Cursors.Default
            Exit Sub
        End If
        ...
        If CDbl(NzZero(Me.TotalOrc.Text)) = 0 Then
            MessageBox.Show("N�o pode finalizar um Or�amento com valor [0,00].", "Valida��o de Or�amento", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.btEnviarCaixa.Enabled = True
            Me.Cursor = Cursors.Default
            Exit Sub
        End If
        ...
        If CDbl(NzZero(vTotal)) <> CDbl(NzZero(Me.TotalOrc.Text)) Then
            MessageBox.Show("O total dos pagamentos est�o diferente do Total do Or�amento, Verifique...", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.ValorVista.Focus()
            Me.btEnviarCaixa.Enabled = True
            Me.Cursor = Cursors.Default
            Exit Sub
        ...
        If Me.CodPgto.Text = "" Then
            MessageBox.Show("O usu�rio deve selecionar uma Condi��o de Pagamento.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.CodPgto.Focus()
            Me.Cursor = Cursors.Default
            Exit Sub
        End If
        ...
        Try
            ...
            Else
                Sql = "Update Orc set C�digoCliente = @1, DataOrc = @2, PercDesconto = @3, VlrDescProduto = @4, ValorProduto = @5, TotalOrc = @6, ValorAVista = @7, ValorOutros = @8, ValorAFaturar = @9, Empresa = @10, FormaPgto = @11, DiasParcelamento = @12, TipoPgto = @13, CodPgto = @14, SenhaGerencia = @15, Contrato = @16, Acrecimo = @17, ImpressoOrcamento = @18, EnderecoEntrega=@19 Where Orc.OrcSequencia = " & Me.OrcSequencia.Text
                ', OrcOperador = @20
        ...
        'Dados do cabe�alho Ok come�a a Salvar os itens
        ...
        MessageBox.Show("Or�amento efetuado com sucesso.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Me.Cursor = Cursors.Default
        Me.CodBarra.Enabled = False
    ...
    Private Sub LocalizarOrc(ByVal CodOrc As String)
        ...
        Sql = "SELECT Orc.*, Clientes.*, Municipio.NomeMunic, UF.NomeUF, Funcion�rios.Nome, FormaPgto.Descri��o FROM ((Orc INNER JOIN ((Clientes LEFT JOIN UF ON Clientes.cUF = UF.CodigoUF) LEFT JOIN Municipio ON Clientes.cMun = Municipio.CodMunicipio) ON Orc.C�digoCliente = Clientes.Codigo) INNER JOIN Funcion�rios ON Orc.C�digoFuncion�rio = Funcion�rios.C�digoFuncion�rio) LEFT JOIN FormaPgto ON Orc.FormaPgto = FormaPgto.CodFormaPgto WHERE (((Orc.OrcSequencia)=" & CodOrc & ") AND ((Orc.Inativo)=False) AND ((Orc.Empresa)=" & CodEmpresa & "));"
        Dim DaVenda As New OleDb.OleDbDataAdapter(Sql, CN)

        'Sql = "SELECT 0 AS Linha, OrcITEM.CodigoProduto, Produtos.Descri��o, OrcITEM.numero, OrcITEM.Qtd, OrcITEM.ValorUnitario, OrcITEM.vAcrecimoVenda, OrcITEM.Desconto, OrcITEM.ValorDesconto, OrcITEM.vDescontoEspecial, OrcITEM.TotalProduto, OrcITEM.CustoMercadoriaVendida FROM OrcITEM INNER JOIN Produtos ON OrcITEM.CodigoProduto = Produtos.CodigoProduto WHERE (((OrcITEM.OrcSequencia)=" & CodOrc & "));"
        ...
        If dsV.Tables("Orc").Rows.Count > 0 Then
            ...
            Me.TpComercio.Text = dsV.Tables("Orc").Rows(0)("TpComercio").ToString

            Me.xCpfCnpj.Text = dsV.Tables("Orc").Rows(0)("CpfCgc").ToString
            Me.xEndereco.Text = dsV.Tables("Orc").Rows(0)("Endere�o").ToString
            ...
            Me.Vendedor.Text = dsV.Tables("Orc").Rows(0)("C�digoFuncion�rio").ToString & "-" & dsV.Tables("Orc").Rows(0)("Funcion�rios.Nome").ToString
        ...
        If Me.Confirmado.Checked = True Then
            ...
            Me.btEnviarCaixa.Enabled = False
            Me.btCriarVenda.Enabled = False
            MessageBox.Show("Esta Or�amento ja foi confirmado e n�o pode mais ser Alterado.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
            Me.ConfImg.Visible = False
            Me.btEnviarCaixa.Enabled = False
            Me.btCriarVenda.Enabled = True
    ...
    Private Sub Lista_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Lista.CellClick
        ...
        If e.ColumnIndex = Me.btExcluirItem.Index AndAlso e.RowIndex >= 0 Then
            ...
            If Me.ImpressoOrcamento.Text = "S" Then
                MessageBox.Show("Este pedido ja foi Impresso e n�o pode mais ser alterado.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If

            If MessageBox.Show("Deseja Excluir o item selecionado", "Valida��o de Dados", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub VerDebitoCliente()
        ...
        'Dim Sql As String = "SELECT Receber.CodCliente AS Cliente, Sum(Receber.ValorDocumento) AS Total FROM Receber WHERE (((Receber.Recebimento) Is Null) AND ((Receber.Baixado)=False) AND ((Receber.Empresa)=" & CodEmpresa & ")) and Receber.inativo = False GROUP BY Receber.CodCliente HAVING (((Receber.CodCliente)=" & Me.xCodCliente.Text & ")); UNION SELECT ArquivoCheque.Cliente, Sum(ArquivoCheque.ValorCh) AS Total FROM ArquivoCheque WHERE (((ArquivoCheque.DataBaixa) Is Null) AND ((ArquivoCheque.Empresa)=" & CodEmpresa & ")) GROUP BY ArquivoCheque.Cliente HAVING (((ArquivoCheque.Cliente)=" & Me.xCodCliente.Text & "));" - incluido contrato
        Dim Sql As String = "SELECT Receber.CodCliente AS Cliente, Sum(Receber.ValorDocumento) AS Total FROM Receber WHERE (((Receber.Recebimento) Is Null) AND ((Receber.Baixado)=False) AND ((Receber.Empresa)=" & CodEmpresa & ")) and Receber.inativo = False AND ((Receber.CodigoContrato) Is Null) GROUP BY Receber.CodCliente HAVING (((Receber.CodCliente)=" & Me.xCodCliente.Text & ")); UNION SELECT ArquivoCheque.Cliente, Sum(ArquivoCheque.ValorCh) AS Total FROM ArquivoCheque WHERE (((ArquivoCheque.DataBaixa) Is Null) AND ((ArquivoCheque.Empresa)=" & CodEmpresa & ")) GROUP BY ArquivoCheque.Cliente HAVING (((ArquivoCheque.Cliente)=" & Me.xCodCliente.Text & "));"
    ...
    Private Sub EncheCodPgto()
        ...
        Sql = "SELECT AnexarForma.Fp_Codigo, [Fp_Codigo] & ' - ' & '(' & [Descri��o] & ')' AS [Desc] FROM AnexarForma INNER JOIN FormaPgto ON AnexarForma.Fp_Codigo = FormaPgto.CodFormaPgto WHERE (((AnexarForma.Tp_Codigo)=" & Me.TipoPgto.SelectedValue & "));"
    ...
    Private Sub AchaCodPgtoSelecionado()
        ...
        Sql = "SELECT AnexarForma.Tp_Codigo, AnexarForma.Fp_Codigo, FormaPgto.Descri��o, FormaPgto.TemEntrada, AnexarForma.Desconto, AnexarForma.Acrescimo FROM AnexarForma INNER JOIN FormaPgto ON AnexarForma.Fp_Codigo = FormaPgto.CodFormaPgto WHERE (((AnexarForma.Tp_Codigo)=" & Me.TipoPgto.SelectedValue & ") AND ((AnexarForma.Fp_Codigo)=" & Me.CodPgto.SelectedValue & "));"
        Dim CMD As New OleDb.OleDbCommand(Sql, Cnn)
        Dim DR As OleDb.OleDbDataReader
        ...
        If DR.HasRows Then
            ...
            If DR.Item("Descri��o") = "A VISTA" Then
                Me.ValorFaturar.Text = FormatNumber(0, 2)
                Me.ValorFaturar.Enabled = False
            Else
                Me.ValorFaturar.Enabled = True
            ...
            'If Me.PedidoTipo.Text = "DEVOLU��O" And Me.Contrato.Text <> "" Then
            '    Me.ValorVista.Enabled = True
            '    Me.ValorCheque.Enabled = False
            '    Me.ValorFaturar.Text = FormatNumber(0, 2)
            '    Me.ValorFaturar.Enabled = False
    ...
    Private Sub TotalDesconto_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TotalDesconto.Validating
        Me.TotalOrc.Text = FormatNumber(CDbl(NzZero(Me.ValorProduto.Text)) + CDbl(NzZero(Me.Acrecimo.Text)) - CDbl(NzZero(Me.TotalDesconto.Text)), 2)
        If Me.PedidoTipo.Text = "DEVOLU��O" Then Me.TotalOrc.Text = NzZero(Me.TotalOrc.Text) * -1
    End Sub
    ...
    Private Sub TotalPedido_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TotalOrc.Validating
        ...
        If CDbl(NzZero(Me.TotalOrc.Text)) < 0 Then
            MessageBox.Show("O valor para o Or�amento n�o pode ser negativo", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.TotalOrc.Clear()
            Me.CodPgto.Focus()
            Exit Sub
        End If
    ...
    Private Sub CodBarra_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles CodBarra.KeyDown
        ...
        If e.KeyCode = Keys.F5 Then
            ...
            Else
                If (Me.CodBarra.Text.IndexOf("*") <> -1) Then
                    If UsarGrade = True Then
                        My.Forms.SellShoesProdutoProcura.ShowDialog()
                    Else
                        My.Forms.ProcuraProduto.ShowDialog()
    ...
    Private Sub Qtd_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Qtd.Leave
        ...
        If NzZero(Me.Qtd.Text) <= 0 Then
            MessageBox.Show("O valor  do item n�o pode ser negativo ou zero", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.Qtd.Text = 1
            Me.Qtd.Focus()
            Exit Sub
        End If
    ...
    Private Sub Unitario_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Unitario.Leave
        If CDbl(NzZero(Me.Unitario.Text)) < CDbl(NzZero(VlrUnitarioTemp)) Then
            MessageBox.Show("O valor do produto n�o pode ser menor que o valor do cadastro.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btCheque_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btCheque.Click
        ...
        If Me.VendaCheque.Checked = False Then
            Me.ValorCheque.Text = FormatNumber(0, 2)
            Me.ValorCheque.Enabled = False
            MessageBox.Show("Este Cliente n�o pode usar esta modalidade de pagamento", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        Else
            Me.ValorCheque.Text = FormatNumber(0, 2)
    ...
    Private Sub btCriarVenda_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btCriarVenda.Click
        ...
        If DiasOrcamento > 10 Then
            MessageBox.Show("Or�amento com mais de 10 dia, favor refazer o or�amento.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
    ...
    Private Sub btImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btImprimir.Click
        ...
        Try
            ...
            Conect.SQL = "SELECT Empresa.Raz�oSocial, Empresa.Endere�o AS EnderecoEmpresa, Empresa.Numero, Empresa.Telefone AS FoneEmpresa, Empresa.Cidade AS CidadeEmpresa, Empresa.Estado AS EstadoEmpresa, Empresa.Cep, Funcion�rios.Nome AS NomeFuncionario, Funcion�rios.Telefone AS FoneFuncionario, Orc.OrcSequencia, Orc.Contrato, Orc.DataOrc, Orc.ValorProduto AS OrcValorProdutos, Orc.VlrDescProduto AS OrcVlrDescProduto, Orc.TotalOrc, Orc.EnderecoEntrega, Clientes.Nome, Clientes.Endere�o, Clientes.Nro, Clientes.Cidade, Clientes.Bairro, Clientes.Estado, Clientes.Telefone, Clientes.Celular,Clientes.Codigo, OrcITEM.CodigoProduto, Produtos.Descri��o AS ProdutoDescricao, Produtos.UnidadeMedida, OrcITEM.Qtd, OrcITEM.ValorUnitario, OrcITEM.ValorDesconto, OrcITEM.TotalProduto, Orc.FormaPgto, FormaPgto.Descri��o, Orc.TipoPgto, TipoPgto.DescricaoTipoPgto FROM TipoPgto INNER JOIN ((((Clientes INNER JOIN (Empresa INNER JOIN (Orc INNER JOIN OrcITEM ON Orc.OrcSequencia = OrcITEM.OrcSequencia) ON Empresa.C�digoEmpresa = Orc.Empresa) ON Clientes.Codigo = Orc.C�digoCliente) INNER JOIN Funcion�rios ON Orc.C�digoFuncion�rio = Funcion�rios.C�digoFuncion�rio) INNER JOIN Produtos ON OrcITEM.CodigoProduto = Produtos.CodigoProduto) INNER JOIN FormaPgto ON Orc.FormaPgto = FormaPgto.CodFormaPgto) ON TipoPgto.CodigoTipoPgto = Orc.TipoPgto WHERE(((Orc.OrcSequencia) = " & Me.OrcSequencia.Text & ")) ORDER BY OrcITEM.Id;"
    ...
    Private Sub btEditar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btEditar.Click
        ...
        If Me.ImpressoOrcamento.Text = "S" Then
            MessageBox.Show("Or�amento ja foi impresso e n�o pode ser mais alterado favor cria outro, o forms do Or�amento sera fechado.", "Valida��o de Dados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.Close()
            Me.Dispose()
            Exit Sub
        End If
