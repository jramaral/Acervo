...
Public Class SellShoesReimpressao
    ...
    Private Sub Grid()
        ...
        Try
            ...
            If Me.rdbNome.Checked = True Then
                Sql = "SELECT Pedido.PedidoSequencia, Pedido.PedidoTipo,  Pedido.DataPedido, Clientes.Nome, Funcionários.Nome AS Vendedor, IIf([Pedido].[Inativo]=False,'','CANCELADO') AS Status,Pedido.TotalPedido,Pedido.DevNumero FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE(((Clientes.Nome) Like '%" & Me.txtProcurar.Text & "%' )) ORDER BY Pedido.PedidoSequencia;"

            End If
            'Por codigo de venda
            If Me.rbnPorcodigo.Checked = True Then
                Sql = "SELECT Pedido.PedidoSequencia, Pedido.PedidoTipo,  Pedido.DataPedido, Clientes.Nome, Funcionários.Nome AS Vendedor, IIf([Pedido].[Inativo]=False,'','CANCELADO') AS Status,Pedido.TotalPedido, Pedido.DevNumero FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE(((Pedido.PedidoSequencia) = " & Me.txtProcurar.Text & " )) ORDER BY Pedido.PedidoSequencia;"
            End If
            'Por Vendedor
            If Me.rbnVendedor.Checked = True Then
                Sql = "SELECT Pedido.PedidoSequencia, Pedido.PedidoTipo,  Pedido.DataPedido, Clientes.Nome, Funcionários.Nome AS Vendedor, IIf([Pedido].[Inativo]=False,'','CANCELADO') AS Status,Pedido.TotalPedido, Pedido.DevNumero FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE(((Funcionários.Nome) Like '%" & Me.txtProcurar.Text & "%' )) ORDER BY Pedido.PedidoSequencia;"
            End If
            'Por data
            If Me.rbnData.Checked = True Then
                Sql = "SELECT Pedido.PedidoSequencia, Pedido.PedidoTipo,  Pedido.DataPedido, Clientes.Nome, Funcionários.Nome AS Vendedor, IIf([Pedido].[Inativo]=False,'','CANCELADO') AS Status,Pedido.TotalPedido, Pedido.DevNumero FROM (Pedido INNER JOIN Clientes ON Pedido.CódigoCliente = Clientes.Codigo) INNER JOIN Funcionários ON Pedido.CódigoFuncionário = Funcionários.CódigoFuncionário WHERE(((Pedido.DataPedido) = #" & Format(CDate(Me.txtProcurar.Text), "MM/dd/yyyy") & " #)) ORDER BY Pedido.PedidoSequencia;"
            End If
    ...
    Private Sub btnVisualizarimpressao_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnVisualizarimpressao.Click
        ...
        If (Dgv.CurrentRow.Cells("gtipovenda").Value = "DEVOLUÇÃO") Then
            cl.strPedOriginal = Dgv.CurrentRow.Cells("coldevnumero").Value.ToString()
        End If
        ...
        'faz impressão da nota promissoria 
