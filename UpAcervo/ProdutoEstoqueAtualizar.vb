Public Class ProdutoEstoqueAtualizar
    ...
    Private Sub btAtualizar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btAtualizar.Click
        ...
        If A1.Checked = True Then
            ...
            Sql = "SELECT [e].CodigoProduto, Sum([e].Qtd) AS SomaDeQtd, (SELECT Sum(s.Qtd) FROM EstoqueUP s WHERE s.Tipo<>'E' AND s.Tipo<>'S' and e.codigoproduto=s.codigoproduto) AS Locado FROM EstoqueUP AS e WHERE ((([e].Tipo) In ('E','S'))) GROUP BY [e].CodigoProduto HAVING ((([e].CodigoProduto)=" & Me.Produto.Text & "));"
        End If

        If A2.Checked = True Then

            Sql = "SELECT e.CodigoProduto, Sum(e.Qtd) AS SomaDeQtd, (SELECT Sum(s.Qtd) FROM EstoqueUP s WHERE s.Tipo<>'E' AND s.Tipo<>'S' and e.codigoproduto=s.codigoproduto) As Locado FROM EstoqueUP e  WHERE (((e.Tipo) IN('E','S'))) GROUP BY e.CodigoProduto;"
        End If
        ...
        Sql = "SELECT CodigoProduto,QuantidadeEstoque,EmLocacao, ControlaEstoque From Produtos"
        ...
        For Each DrEstoque In Ds.Tables("EstoqueUP").Rows
            ...
            Dim expression = String.Format("CodigoProduto={0} And ControlaEstoque like 'SIM'", DrEstoque("CodigoProduto").ToString)
            DrProdutos = Ds.Tables("Produtos").Select(expression)

            If DrProdutos.Length <> 0 Then

                DrProdutos(0).BeginEdit()
                DrProdutos(0)("QuantidadeEstoque") = DrEstoque("SomaDeQtd")
                DrProdutos(0)("EmLocacao") = nzNUM(DrEstoque("Locado"))
